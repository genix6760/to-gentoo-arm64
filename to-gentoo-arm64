...
if [ "$(uname -m)" != "aarch64" ]; then
  echo "Only arm64 is supported. Your system is $(uname -m)." >&2
  exit 1
fi
...
STAGE3=$(curl -s "${MIRROR}/releases/arm64/autobuilds/latest-stage3-arm64-systemd.txt" | grep stage3-amd64-systemd | cut -d ' ' -f 1)
if [ -z "${STAGE3}" ]; then
...
wget "${MIRROR}releases/arm64/autobuilds/${STAGE3}"
...
if [ -e "${GENTOO}/bin/busybox" ]; then
  echo "using static busybox from Gentoo to run mv"
...
echo "You are now running Gentoo!"
echo "Please run now: source /etc/profile"
echo "It is advisable to install a new kernel and install and configure grub."
echo "DONE! You should reboot afterwards."